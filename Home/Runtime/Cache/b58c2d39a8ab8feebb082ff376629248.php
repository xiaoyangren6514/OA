...
<title>文件列表|<?php echo ($configcache['Title']); ?></title>
...
$(document).ready(function() {
    $('#content h2 .add').click(function() {
		popload('添加文件分类目录',600,340,'__APP__/File/fileclass_add/?sid=<?php echo ($fileinfo["sid"]); ?>');
		addDiv($('#iframe_pop'));
		popclose();
	});
	//快捷导航跳转
	...
	//加载文件跟文件夹
	$.get('__APP__/File/fileajax?sid=<?php echo ($fileinfo["sid"]); ?>',function(data) {
		//$('#wintq').remove();
		//$('#zbody').remove();
		$('.content').html(data);
		//判断是否存在文件夹
...
<div id="content">
    ...
    <h2>
    	<div class="h2_left">
            ...
            <select name="sid" class="select">
            	...
            	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["ID"]); ?>" <?php if($vo["ID"] == $fileinfo['sid']): ?>selected<?php endif; ?> ><?php if($vo["Sid"] != 0): ?>└<?php endif; echo ($vo["html"]); echo ($vo["classname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
            &nbsp;&nbsp;&nbsp;&nbsp;快捷上传：
            <select name="sid" class="select">
            	<option value="0">顶级目录</option>
    ...
    <h3>
    	...
    	<a href="__APP__/File/uploadpic?sid=<?php echo ($fileinfo['sid']); ?>">上传图片</a>
        <a href="__APP__/File/uploadfile?sid=<?php echo ($fileinfo['sid']); ?>">上传文件</a>
        <a href="__APP__/File/fileshare">共享文件</a>
    </h3>
    <div class="content"></div>
    <div id="page">对选中文件项进行&nbsp;&nbsp;<a href="javascript:;" id="fileselect">全选</a><a href="javascript:;" id="dely">删除</a>(不包括文件夹)&nbsp;&nbsp;&nbsp;&nbsp;当前共有 <strong><?php echo ($fileinfo['count']); ?></strong> 个文件或文件夹&nbsp;&nbsp;&nbsp;&nbsp;<strong>云端文件统计 —— </strong>文件总数：<strong><?php echo ($fileinfo['filecount']); ?></strong> 个&nbsp;&nbsp;&nbsp;&nbsp;占用空间：<strong><?php echo (round($fileinfo['filesum']/1024/1024,2)); ?>M</strong>&nbsp;&nbsp;&nbsp;&nbsp;总容量/剩余：<strong><?php echo ($fileinfo['capacity']); ?>M / <?php echo ($fileinfo['scapacity']); ?>M</strong></div>
